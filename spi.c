...
static int spi_open (struct inode *inode, struct file *filp)
{
    ...
    memset(&spiinfo[idev], 0, sizeof(spi_device_t));
    
    if(filp->f_flags & O_NONBLOCK)
        spiinfo[idev].nonblock = 1;
/*  This way doesn't work. 
    init_waitqueue_head(&(spiinfo[idev].tx_wq));
    init_waitqueue_head(&(spiinfo[idev].rx_wq));
*/
    if(idev == 0)
    {
        spiinfo[0].tx_wq = &spitxq0;
        spiinfo[0].rx_wq = &spirxq0;
        spiinfo[0].regbase = SPI0_REGBASE;
    }
    else
    {
        spiinfo[1].tx_wq = &spitxq1;
        spiinfo[1].rx_wq = &spirxq1;
        spiinfo[1].regbase = SPI1_REGBASE;
    }
    spiinfo[idev].opened = 1;
    spiinfo[idev].phase = 1;
    spiinfo[idev].bdrate = SPI_DEFAULT_BARD;
    if(idev)
    {
        strcpy(intname, SPI_INT0NAME);
        spiinfo[idev].irqnum = SPI1_IRQ_NUM;
    }
    else
    {
        strcpy(intname, SPI_INT1NAME);
        spiinfo[idev].irqnum = SPI0_IRQ_NUM;
    }    

    filp->private_data = &spiinfo[idev];
